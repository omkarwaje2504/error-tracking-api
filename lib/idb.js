...
// Bumped for the new 'printers' store — IndexedDB only runs onupgradeneeded
// (which creates missing object stores) when the version number goes up,
// so adding a store name here without bumping this leaves it missing in
// every browser that already has an older-versioned local DB.
const DB_VERSION = 2;
const STORES = ['companies', 'brands', 'productTypes', 'printers', 'users', '_meta'];
