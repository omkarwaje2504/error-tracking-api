...
// deterministic order here instead. Newest first, matching every other
// table in the app (tasks, projects, credentials, ...).
...
export async function getReference(storeName) {
    ...
    const lastSync = await idbGetMeta(metaKey).catch(() => null);
    ...
    try {
        ...
        const cached = await idbGetAll(storeName).catch(() => null);
        return sortByCreatedAt(cached || []);
    }

    const syncedAt = new Date().toISOString();

    // The network fetch above already succeeded — that's the data we're
    // returning either way. Failing to *cache* it locally (a schema
    // mismatch, a blocked/broken IndexedDB, private browsing) shouldn't
    // take the whole call down; just skip caching and hand back what we got.
    try {
        if (!since) {
        ...
        return sortByCreatedAt(all && all.length ? all : toUpsert);
    } catch {
        return sortByCreatedAt(delta.filter((r) => !r.deleted));
    }
}
