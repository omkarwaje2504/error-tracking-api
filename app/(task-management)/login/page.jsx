...
export default function LoginPage() {
    ...
    const [form, setForm] = useState({ name: '', email: '', mobile: '', password: '', role: 'designer', team: 'graphic' });
            ...
            <div className="w-full max-w-[420px]">
                ...
                {mode === 'register' && (
                    <>
                        ...
                        <Field label="Mobile">
                            <input className="field" value={form.mobile} onChange={(e) => update('mobile', e.target.value)} />
                        </Field>
                        <div className="mb-4 flex gap-3">
                            ...
                            <Field label="Team" className="mb-0 flex-1">
                                <select className="field" value={form.team} onChange={(e) => update('team', e.target.value)}>
                ...
                {mode === 'login' && (
                    <label className="mb-5 mt-1 flex cursor-pointer items-center gap-2 text-sm text-neutral-400">
                        <input type="checkbox" className="h-4 w-4 rounded border-line bg-panel2 accent-white" />
                        Stay logged in
                    </label>
                )}
                ...
                {mode === 'login' && (
                    <p className="mt-5 text-center text-sm font-semibold text-neutral-200">
                        <span className="cursor-pointer hover:text-white">I forgot my password</span>
                    </p>
                )}

                <p className="mt-4 text-center text-sm text-neutral-500">
                    {mode === 'login' ? "Don't you have an account? " : 'Have an account? '}
                    <span
...
function Field({ label, children, className = 'mb-4' }) {
    return (
        <div className={className}>
            <label className="mb-1.5 block text-sm font-semibold text-neutral-200">{label}</label>
            {children}
        </div>
    );
}
