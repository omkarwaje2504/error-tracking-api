...
export default function Sidebar({ user, onAdd }) {
    ...
    const nav = (
        <div className="flex h-full flex-col p-5">
            ...
            <nav className="flex flex-col gap-1">
                {links.map((l) => (
                    <Link
                        ...
                        className={`rounded-xl px-3.5 py-2.5 text-sm transition-colors ${pathname === l.href
            ...
            <button className="btn-ghost mt-auto" onClick={logout}>Logout</button>
        </div>
    );
    ...
    return (
        <>
            ...
            <header className="sticky top-0 z-30 flex items-center justify-between border-b border-line bg-bg/80 px-4 py-3 backdrop-blur md:hidden">
                ...
                <button className="btn-ghost !px-3 !py-1.5" onClick={() => setOpen(true)}>☰</button>
            </header>
